...
export class InstituteRepository {
  ...
  async findAll(country?: string, name?: string): Promise<Institute[]> {
    const queryBuilder =
      this.instituteRepository.createQueryBuilder('institute');
      ...
      try {
        ...
        if (country) {
          queryBuilder.andWhere(
            'similarity(LOWER(institute.name), LOWER(:name)) > :threshold',
            {
              name: name,
              threshold: similarityThreshold,
            },
        ...
        queryBuilder.orderBy(
          'similarity(LOWER(institute.name), LOWER(:name))',
          'DESC',
        );
      } catch (error) {
        // Fallback to LIKE search if pg_trgm extension is not available
        console.warn(
          'pg_trgm extension not available, falling back to LIKE search',
        );
        if (country) {
          queryBuilder.andWhere('LOWER(institute.name) LIKE LOWER(:name)', {
            name: `%${name}%`,
  ...
  async findAllWithEnhancedFuzzy(
    country?: string,
    name?: string,
    similarityThreshold: number = 0.3,
  ): Promise<Institute[]> {
      ...
      try {
        ...
        if (searchTerms.length === 1) {
          ...
          const conditions = searchTerms.map(
            (term, index) => `LOWER(institute.name) LIKE LOWER(:term${index})`,
          );
        ...
        console.warn(
          'pg_trgm extension not available, falling back to enhanced LIKE search',
        );
        const searchTerms = name
          .trim()
          .split(/\s+/)
