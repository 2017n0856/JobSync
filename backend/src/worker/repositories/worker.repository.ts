...
export class WorkerRepository {
  ...
  async findAll(filters?: GetWorkerQueryDto): Promise<Worker[]> {
    ...
    if (filters?.country) {
      if (filters.name) {
        queryBuilder.andWhere('worker.country = :country', { country: filters.country });
      } else {
        queryBuilder.where('worker.country = :country', { country: filters.country });
      }
    }
    ...
    if (filters?.specialties && filters.specialties.length > 0) {
      const whereClause = filters.name || filters.country || filters.instituteName ? 'AND' : 'WHERE';
      // Filter workers who have any of the specified specialties
      const specialtyConditions = filters.specialties.map((_, index) => 
        `worker.specialties LIKE :specialty${index}`
      );
      const specialtyParams = filters.specialties.reduce((acc, specialty, index) => {
        acc[`specialty${index}`] = `%${specialty}%`;
        return acc;
      }, {} as Record<string, string>);
      
      queryBuilder.andWhere(`(${specialtyConditions.join(' OR ')})`, specialtyParams);
    }
    
    return await queryBuilder.orderBy('worker.name', 'ASC').getMany();
  }

  async update(id: number, updateData: Partial<Worker>): Promise<Worker | null> {
